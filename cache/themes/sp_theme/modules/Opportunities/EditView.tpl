...
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_TIPO_VENTA_C' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.tipo_venta_c.name}"
id="{$fields.tipo_venta_c.name}"
title='Tipo de Venta'           
>
{if isset($fields.tipo_venta_c.value) && $fields.tipo_venta_c.value != ''}
{html_options options=$fields.tipo_venta_c.options selected=$fields.tipo_venta_c.value}
{else}
{html_options options=$fields.tipo_venta_c.options selected=$fields.tipo_venta_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.tipo_venta_c.options }
{capture name="field_val"}{$fields.tipo_venta_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.tipo_venta_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.tipo_venta_c.name}"
id="{$fields.tipo_venta_c.name}"
title='Tipo de Venta'          
>
{if isset($fields.tipo_venta_c.value) && $fields.tipo_venta_c.value != ''}
...
<input
id="{$fields.tipo_venta_c.name}-input"
name="{$fields.tipo_venta_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.tipo_venta_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.tipo_venta_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.tipo_venta_c.name}-input', '{$fields.tipo_venta_c.name}');sync_{$fields.tipo_venta_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
    var selectElem = document.getElementById("{/literal}{$fields.tipo_venta_c.name}{literal}");
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
    ...
    SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.tipo_venta_c.name}-input');
    SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.tipo_venta_c.name}-image');
    SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.tipo_venta_c.name}');
    ...
    //global variable
    sync_{/literal}{$fields.tipo_venta_c.name}{literal} = function(){
        SyncToHidden();
    }
    function syncFromHiddenToWidget(){
        ...
        for (i=0;i<selectElem.options.length;i++){
            ...
            if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.tipo_venta_c.name}-input{literal}'))
                SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
        }
    }

    YAHOO.util.Event.onAvailable("{/literal}{$fields.tipo_venta_c.name}{literal}", syncFromHiddenToWidget);
    {/literal}
...
<script type="text/javascript">
    ...
    addLoadEvent(function(){literal}{loadDynamicEnum({/literal}"","{$fields.tipo_venta_c.name}"{literal})}{/literal});
    if (SUGAR.ajaxUI && SUGAR.ajaxUI.hist_loaded) {literal}{loadDynamicEnum({/literal}"","{$fields.tipo_venta_c.name}"{literal})}{/literal}
</script>
</div>  
{counter name="fieldsUsed" start=0 print=false assign="fieldsUsed"}
<div class="form-group col-md-6" >
...
id='{$fields.probability.name}' style="max-width:170px;" size='30'  value='{sugar_number_format precision=0 var=$value}' title='' tabindex='0'    >
</div>  
<div class="form-group col-md-6" >
<label>
...
{capture assign="other_attributes"}alt="{$APP.LBL_ENTER_DATE}" style=" padding-left:6px" border="0" id="{$fields.date_closed.name}_trigger"{/capture}
...
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_UNIDAD_INDUSTRIAL_C' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
id='{$fields.unidad_industrial_c.name}' style="max-width:170px;" size='30' maxlength='255' value='{sugar_number_format precision=0 var=$value}' title='Unidad industrial (UV)' tabindex='0'    >
</div>  
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_MANZANO_C' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_UBICACION_C' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
maxlength='255' 
value='{$value}' title='Ubicaci&oacute;n (Lote)'       >
</div>  
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_SUPERFICIE' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
{if strlen($fields.superficie_c.value) <= 0}
{assign var="value" value=$fields.superficie_c.default_value }
{else}
{assign var="value" value=$fields.superficie_c.value }
{/if}  
<input type='text' name='{$fields.superficie_c.name}' 
id='{$fields.superficie_c.name}' style="max-width:170px;" size='30' maxlength='255' value='{sugar_number_format precision=0 var=$value}' title='Superficie (m2)' tabindex='0'    >
</div>  
</div>   
...
<div class="form-group col-md-6" >
<label>
{capture name="label" assign="label"}{sugar_translate label='LBL_MONEDA_C' module='Opportunities'}{/capture}
{$label|strip_semicolon}:
</label>
{counter name="fieldsUsed"}
...
addToValidate('EditView', 'ubicacion_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_UBICACION_C' module='Opportunities' for_js=true}{literal}' );
addToValidate('EditView', 'manzano_c', 'int', false,'{/literal}{sugar_translate label='LBL_MANZANO_C' module='Opportunities' for_js=true}{literal}' );
addToValidate('EditView', 'tipo_venta_c', 'dynamicenum', false,'{/literal}{sugar_translate label='LBL_TIPO_VENTA_C' module='Opportunities' for_js=true}{literal}' );
...
addToValidate('EditView', 'superficie_c', 'int', false,'{/literal}{sugar_translate label='LBL_SUPERFICIE_C' module='Opportunities' for_js=true}{literal}' );
addToValidate('EditView', 'moneda_c', 'dynamicenum', false,'{/literal}{sugar_translate label='LBL_MONEDA_C' module='Opportunities' for_js=true}{literal}' );
addToValidate('EditView', 'lbl_tipo_pago_c', 'dynamicenum', false,'{/literal}{sugar_translate label='LBL_LBL_TIPO_PAGO_C' module='Opportunities' for_js=true}{literal}' );
addToValidate('EditView', 'unidad_industrial_c', 'int', false,'{/literal}{sugar_translate label='LBL_UNIDAD_INDUSTRIAL_C' module='Opportunities' for_js=true}{literal}' );
