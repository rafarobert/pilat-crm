   ...
   {{counter name="tabCount" start=-1 print=false assign="tabCount"}}     
    {{foreach name=section from=$sectionPanels key=label item=panel}}
        ...
        {{if (isset($tabDefs[$label_upper].newTab) && $tabDefs[$label_upper].newTab == true)}}
          <li class="{{if $tabCount == 0}}active{{/if}}"><a id="tab{{$tabCount}}" href="#tab-{{$tabCount}}" data-toggle="tab"><em>{sugar_translate label='{{$label}}' module='{{$module}}'}</em></a></li>
       {{/if}}
    {{/foreach}}
    </ul>
...
{{foreach name=section from=$sectionPanels key=label item=panel}}
 ...
 {{if $useTabs}}  
   ...
   {{if (isset($tabDefs[$label_upper].newTab) && $tabDefs[$label_upper].newTab == true)}} 
       ...
       {{/if}}    
      <div class="tab-pane {{if $tabCount == 0}}active{{/if}}" id="tab-{{$tabCount}}">
   {{/if}}     
 {{/if}}      
...
{counter name="fieldsUsed" start=0 print=false assign="fieldsUsed"}
	...
	{{foreach name=colIteration from=$rowData key=col item=colData}}
     	{{if count($rowData) == 1}}
        <div class="form-group col-md-12" {{if $colData.field.name=="billing_address_street"}}style="clear:both;"{{/if}}>
    	{{else}}
        <div class="form-group col-md-6" {{if $colData.field.name=="billing_address_street"}}style="clear:both;"{{/if}}>
     	{{/if}}
