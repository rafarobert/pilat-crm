...
{assign var="cards_cols" value=0}
{assign var="pathtplcards" value=$pageData.bean.moduleDir}
{assign var="cookname" value="view_$pathtplcards"}
{assign var="pathtplcards" value="custom/modules/$pathtplcards/cardview4.tpl"}
{if file_exists($pathtplcards)} 
    {assign var="cards_cols" value=4}
    {assign var="cards_colssm" value=3}
{else}
    {assign var="pathtplcards" value=$pageData.bean.moduleDir}
    {assign var="pathtplcards" value="custom/modules/$pathtplcards/cardview2.tpl"}
    {if file_exists($pathtplcards)} 
       {assign var="cards_cols" value=2}
       {assign var="cards_colssm" value=6}
    {/if}   
{/if}   

{if $cards_cols>0 and $smarty.cookies.$cookname == 'cards'} 
<div class="hpanel">
  <div class="panel-body" style="background-color:transparent;">
   ...
   {foreach name=rowIteration from=$data key=id item=rowData}  
        ...
        {if   ($smarty.foreach.rowIteration.iteration-1) is div by $cards_cols}
            {if $smarty.foreach.rowIteration.iteration<>1}
             </div>
            {/if}
             <div class="row">
                
        {/if}
         <div class="col-sm-{$cards_colssm}">
           {include file=$pathtplcards}
         </div>  
    {foreachelse}
        <div>
        ...
        {assign var="action_menu_location" value="top"}
            ...
            <tr>
                ...
                {assign var='datahide' value="phone"}
                            ...
                            {if $params.default != 1}<th  data-hide="all">{else}
                              {if $params.name != "name"}
