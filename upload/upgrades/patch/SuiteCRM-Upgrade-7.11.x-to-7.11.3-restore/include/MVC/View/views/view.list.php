...
class ViewList extends SugarView
{
        ...
        if (!empty($_REQUEST['displayColumns'])) {
            foreach (explode('|', $_REQUEST['displayColumns']) as $num => $col) {
                if (!empty($this->listViewDefs[$module][$col]))
                {
                    ...
                    $displayColumns[$col]['default']=1;
                }
            }
            foreach ($this->listViewDefs[$module] as $col => $this->params) {
                  if(!isset($displayColumns[$col]))
   ...
   $this->lv->ss->assign('savedSearchData', $this->searchForm->getSavedSearchData());
            ...
            if(!empty($calendar_setting))
                ...
                {
                    if(($modc->module==$this->module)&&($_COOKIE["view_".$this->module]=='calendar'))
                      $view_sp='calendar';
                }
            ...
            if(!empty($edit_setting))
                ...
                {
                    if(($modc==$this->module)&&($_COOKIE["view_".$this->module]=='edit'))
                      $view_sp='edit';
                }
            ...
            if(!empty($kanban_setting))
                ...
                {
                    if(($modc->module==$this->module)&&($_COOKIE["view_".$this->module]=='kanban'))
                      $view_sp='kanban';
                }
             ...
             if($view_sp=='calendar')
                {
                  $this->lv->setup($this->seed, 'include/ListView/ListViewGenericCalendar.tpl', $this->where, $this->params);
                }
             if($view_sp=='edit')
               {
                     $this->lv->setup($this->seed, 'include/ListView/ListViewGenericEdit.tpl', $this->where, $this->params);                           
               }
            if($view_sp=='kanban')   
               {                                           
                  $this->lv->setup($this->seed, 'include/ListView/ListViewGenericKanban.tpl', $this->where, $this->params);        
                }
            if($view_sp=='list')   
               {                                     
