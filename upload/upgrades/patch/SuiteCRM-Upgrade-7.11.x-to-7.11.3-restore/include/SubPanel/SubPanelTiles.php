...
<div id="subpanel_hpanel" class="hpanel"><ul class="nav nav-tabs">
<?php
 ...
 // this array will store names of sub-panels that can contain items
    ...
    $toolb_setting="";
    if(!empty($sugar_config['sp_theme_subpanel_setting']))
       $toolb_setting = json_decode($sugar_config['sp_theme_subpanel_setting']);


    $viewtoolbar=false;    
    if(!empty($toolb_setting))
        foreach($toolb_setting as $cid)
        {
            if($cid==$this->module)
            {
             $viewtoolbar=true;
    ...
    if(($viewtoolbar)&&($showContainer)&&(empty($selected_group)))
    {
  
        $ct=0;
        if(empty($_COOKIE[$this->module.'_subpanel']))
            $active="active";  
        if(!empty($GLOBALS['sugar_config']['max_subpanel_toolbar']))
           $maxsubp=$GLOBALS['sugar_config']['max_subpanel_toolbar'];
        else
          $maxsubp=12;
        foreach ($tabs as $tab)
        { 
            ...
            if ($thisPanel === false)
                    continue;
          if((empty($active))&&(isset($_COOKIE[$this->module.'_subpanel'])))
            if($_COOKIE[$this->module.'_subpanel']=='subpanel_'.$tab)
               $active="active";
            
          if($ct==$maxsubp)
          {
          echo '<li  class="dropdown '.$active.'"><a class="dropdown-toggle" data-toggle="dropdown" href="#">'.$app_strings['LBL_MORE'].'<span class="caret"></span></a><ul class="dropdown-menu">'; 
          }
    
          echo '<li class="'.$active.'"><a href="#subpanel_' . $tab . '" data-toggle="tab">'. $thisPanel->get_title().'</a></li>';
          $ct++;
          $active="";  
        }  
...
$display= 'none';
	...
	//				$div_display = 'none';
		//		}
            ...
            $t=$thisPanel->get_title();
            if(($showContainer)&&(empty($selected_group))) 
            {
            if(!$viewtoolbar)
              echo '<div class="" id="subpanel_'.$tab.'">';
            else
              echo '<div class="tab-pane subpanclass '.$active.'" id="subpanel_'.$tab.'">';
            echo '<div class="panel-header">
             ...
             {
             echo '<div class="tab-pane subpanclasswithtab '.$active.'" id="subpanel_'.$tab.'">
