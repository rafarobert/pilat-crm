...
class actionSendEmail extends actionBase {
	...
	$Email_Type = $emailTemp->type;
        ...
        if (trim($Email_Type) === "sms" || trim($Email_Type) === "whatsapp") {
            if ($emailTemp->id == '') {
                return false;
            }
            ...
            if (isset($params['individual_email']) && $params['individual_email']) {
                ...
                foreach ($emails['to'] as $email_to) {
                    ...
                    if($Email_Type === "whatsapp"){			
			$GLOBALS['log']->fatal(array($email_to));
			$GLOBALS['log']->fatal($emailTemp->subject);
			$GLOBALS['log']->fatal($emailTemp->body_html);
			$GLOBALS['log']->fatal($emailTemp->body);
			$GLOBALS['log']->fatal($emails['cc']);
			$GLOBALS['log']->fatal($emails['bcc']);
                        $this->DT_Sending_Whatsapp(array($email_to), $emailTemp->subject, $emailTemp->body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments, $Email_TNAME);
                    }else
                    {
			...
			$GLOBALS['log']->fatal($emails['cc']);
			$GLOBALS['log']->fatal($emails['bcc']);
                return $this->DT_Sending_Whatsapp($emails['to'], $emailTemp->subject, $email_body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments, $Email_TNAME);
                }else
                {
            ...
            if (isset($params['individual_email']) && $params['individual_email']) {
                ...
                $this->parse_template($bean, $emailTemp);
		$GLOBALS['log']->fatal(print_r($emailTemp,true));
                if ($emailTemp->text_only == '1') {
                    $email_body_html = $emailTemp->body;
                } else {
                    $email_body_html = $emailTemp->body_html;
    ...
    function DT_Sending_Whatsapp($emailTo, $emailSubject, $emailBody, $altemailBody, SugarBean $relatedBean = null, $emailCc = array(), $emailBcc = array(), $attachments = array(), $Email_TNAME) {
        ...
        foreach ($emailTo as $to) {
            ...
            //$whatsapp_config_waboxapp_mobile_number = $settings_whatsapp->settings['whatsapp_config_waboxapp_mobile_number']; 
            if(strlen($to) == 8){
                $to = '591'.$to;
            }
            $final_url = "https://www.waboxapp.com/api/send/chat?token=".$whatsapp_config_waboxapp_token."&uid=".$whatsapp_config_waboxapp_mobile_number."&to=".trim($to)."&custom_uid=".$message_id."&text=".$message_p;
            $ch = curl_init();
            ...
            $dt_whatsapp_utils->create_whatsapp($Email_TNAME, $altemailBody, $call_status, $relatedBean->module_dir, $relatedBean->id,$message_id);        
        }
        return true;
    }
