$(document).ready(function (){
    ...
    $('#phone_home').after('<div id="record_phone_home" style="color:red;"></div>');
	...
	//$('#Contacts0emailAddressRow0').parent().parent().parent().after('<div id="record_email" style="color:red;display:none;float: left;margin-right: -5%;margin-top:-14px; margin-left: 15px;"><br></div>');
    ...
    $('#phone_home').blur(function () {
   	...
   	/*$('#Contacts0emailAddress0').blur(function () {
...
{	
	...
	$.ajax(
    {
        ...
        data:
        {
            action: 	'Check_Duplicate_Contact_Name',
            module: 	'Contacts',
            div_name: 	'div_name',
...
{   
    ...
    $.ajax(
    {
        ...
        data:
        {
            action:     'Check_Duplicate_Contact_Document_Number',
            module:     'Contacts',
            div_name:   'div_doc_no',
            doc_no:     doc_no,
            record_id:  record_id
...
{	
    ...
    $.ajax(
    {
        ...
        data:
        {
            action:     'Check_Duplicate_Contact_Phone',
            module:     'Contacts',
            div_name:   div_popup,
            phone_no:   phone_no,
            field_id:   field_id,
        ...
        success: function(get_body)
        {
            if(get_body!= "")
            {
                ...
                if(field_id == "phone_home")
                {
                    if(get_body != "No Duplicate Found")
                    {
                        $("#"+div_id).html(get_body + '&nbsp;<span id="'+div_id+'" onclick="openpopup(\'div_home_phone\')" style="position:relative;"><img vertical-align="middle" class="info" border="0" src="modules/Duplicate_Preventive/info_inline.png"></span>');
                    }
                    else
                    {
                        $("#"+div_id).html("");
...
{   
    var contact_email = $('#Contacts0emailAddress0').val();
    $.ajax(
    {
        url: 'index.php?entryPoint=Prevent_Duplicate',
        type: 'POST',
        data:
        {
            action:     'Check_Duplicate_Contact_Email',
            module:     'Contacts',
            div_name:   'div_email',
            email:      contact_email,
            record_id:  record_id
