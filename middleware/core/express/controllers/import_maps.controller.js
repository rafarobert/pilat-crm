...
importMapsCtrl.getAllImportMaps = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.importMaps.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objImportMaps = await importMapService.getAllImportMaps(req.query);
        if (objImportMaps && objImportMaps.rows && objImportMaps.count) {
            util.setSuccess(200, 'ImportMaps retrieved', objImportMaps.rows, objImportMaps.count, req.query.limit, req.query.offset);
        } else {
