...
notesCtrl.getAllNotes = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.notes.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objNotes = await noteService.getAllNotes(req.query);
        if (objNotes && objNotes.rows && objNotes.count) {
            util.setSuccess(200, 'Notes retrieved', objNotes.rows, objNotes.count, req.query.limit, req.query.offset);
        } else {
