...
cronRemoveDocumentsCtrl.getAllCronRemoveDocuments = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.cronRemoveDocuments.rawAttributes,req.body);
        ...
        if (objCronRemoveDocuments && objCronRemoveDocuments.rows && objCronRemoveDocuments.count) {
            util.setSuccess(200, 'CronRemoveDocuments retrieved', objCronRemoveDocuments.rows, objCronRemoveDocuments.count, req.query.limit, req.query.offset);
        } else {
