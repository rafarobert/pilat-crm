...
meetingsCstmCtrl.getAllMeetingsCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.meetingsCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objMeetingsCstm = await meetingCstmService.getAllMeetingsCstm(req.query);
        if (objMeetingsCstm && objMeetingsCstm.rows && objMeetingsCstm.count) {
            util.setSuccess(200, 'MeetingsCstm retrieved', objMeetingsCstm.rows, objMeetingsCstm.count, req.query.limit, req.query.offset);
        } else {
