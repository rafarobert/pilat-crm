...
aodIndexAuditCtrl.getAllAodIndexAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aodIndexAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAodIndexAudit = await aodIndexAuditService.getAllAodIndexAudit(req.query);
        if (objAodIndexAudit && objAodIndexAudit.rows && objAodIndexAudit.count) {
            util.setSuccess(200, 'AodIndexAudit retrieved', objAodIndexAudit.rows, objAodIndexAudit.count, req.query.limit, req.query.offset);
        } else {
