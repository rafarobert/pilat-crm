...
pilatParamsCtrl.getAllPilatParams = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.pilatParams.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objPilatParams = await pilatParamService.getAllPilatParams(req.query);
        if (objPilatParams && objPilatParams.rows && objPilatParams.count) {
            util.setSuccess(200, 'PilatParams retrieved', objPilatParams.rows, objPilatParams.count, req.query.limit, req.query.offset);
        } else {
