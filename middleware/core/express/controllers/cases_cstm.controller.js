...
casesCstmCtrl.getAllCasesCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.casesCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCasesCstm = await caseCstmService.getAllCasesCstm(req.query);
        if (objCasesCstm && objCasesCstm.rows && objCasesCstm.count) {
            util.setSuccess(200, 'CasesCstm retrieved', objCasesCstm.rows, objCasesCstm.count, req.query.limit, req.query.offset);
        } else {
