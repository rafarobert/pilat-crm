...
upgradeHistoryCtrl.getAllUpgradeHistory = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.upgradeHistory.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objUpgradeHistory = await upgradeHistoryService.getAllUpgradeHistory(req.query);
        if (objUpgradeHistory && objUpgradeHistory.rows && objUpgradeHistory.count) {
            util.setSuccess(200, 'UpgradeHistory retrieved', objUpgradeHistory.rows, objUpgradeHistory.count, req.query.limit, req.query.offset);
        } else {
