...
aopCaseUpdatesAuditCtrl.getAllAopCaseUpdatesAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aopCaseUpdatesAudit.rawAttributes,req.body);
        ...
        if (objAopCaseUpdatesAudit && objAopCaseUpdatesAudit.rows && objAopCaseUpdatesAudit.count) {
            util.setSuccess(200, 'AopCaseUpdatesAudit retrieved', objAopCaseUpdatesAudit.rows, objAopCaseUpdatesAudit.count, req.query.limit, req.query.offset);
        } else {
