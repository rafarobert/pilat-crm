...
aorReportsCtrl.getAllAorReports = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aorReports.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAorReports = await aorReportService.getAllAorReports(req.query);
        if (objAorReports && objAorReports.rows && objAorReports.count) {
            util.setSuccess(200, 'AorReports retrieved', objAorReports.rows, objAorReports.count, req.query.limit, req.query.offset);
        } else {
