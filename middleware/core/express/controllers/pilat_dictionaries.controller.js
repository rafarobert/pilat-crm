...
pilatDictionariesCtrl.getAllPilatDictionaries = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.pilatDictionaries.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objPilatDictionaries = await pilatDictionaryService.getAllPilatDictionaries(req.query);
        if (objPilatDictionaries && objPilatDictionaries.rows && objPilatDictionaries.count) {
            util.setSuccess(200, 'PilatDictionaries retrieved', objPilatDictionaries.rows, objPilatDictionaries.count, req.query.limit, req.query.offset);
        } else {
