...
meetingsCtrl.getAllMeetings = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.meetings.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objMeetings = await meetingService.getAllMeetings(req.query);
        if (objMeetings && objMeetings.rows && objMeetings.count) {
            util.setSuccess(200, 'Meetings retrieved', objMeetings.rows, objMeetings.count, req.query.limit, req.query.offset);
        } else {
