...
securitygroupsRecordsCtrl.getAllSecuritygroupsRecords = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.securitygroupsRecords.rawAttributes,req.body);
        ...
        if (objSecuritygroupsRecords && objSecuritygroupsRecords.rows && objSecuritygroupsRecords.count) {
            util.setSuccess(200, 'SecuritygroupsRecords retrieved', objSecuritygroupsRecords.rows, objSecuritygroupsRecords.count, req.query.limit, req.query.offset);
        } else {
