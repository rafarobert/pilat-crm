...
emailsBeansCtrl.getAllEmailsBeans = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emailsBeans.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmailsBeans = await emailBeanService.getAllEmailsBeans(req.query);
        if (objEmailsBeans && objEmailsBeans.rows && objEmailsBeans.count) {
            util.setSuccess(200, 'EmailsBeans retrieved', objEmailsBeans.rows, objEmailsBeans.count, req.query.limit, req.query.offset);
        } else {
