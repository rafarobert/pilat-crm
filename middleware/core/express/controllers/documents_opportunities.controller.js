...
documentsOpportunitiesCtrl.getAllDocumentsOpportunities = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.documentsOpportunities.rawAttributes,req.body);
        ...
        if (objDocumentsOpportunities && objDocumentsOpportunities.rows && objDocumentsOpportunities.count) {
            util.setSuccess(200, 'DocumentsOpportunities retrieved', objDocumentsOpportunities.rows, objDocumentsOpportunities.count, req.query.limit, req.query.offset);
        } else {
