...
projectCtrl.getAllProject = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.project.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objProject = await projectService.getAllProject(req.query);
        if (objProject && objProject.rows && objProject.count) {
            util.setSuccess(200, 'Project retrieved', objProject.rows, objProject.count, req.query.limit, req.query.offset);
        } else {
