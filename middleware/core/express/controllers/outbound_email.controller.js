...
outboundEmailCtrl.getAllOutboundEmail = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.outboundEmail.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objOutboundEmail = await outboundEmailService.getAllOutboundEmail(req.query);
        if (objOutboundEmail && objOutboundEmail.rows && objOutboundEmail.count) {
            util.setSuccess(200, 'OutboundEmail retrieved', objOutboundEmail.rows, objOutboundEmail.count, req.query.limit, req.query.offset);
        } else {
