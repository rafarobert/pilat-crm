...
aokKnowledgebaseAuditCtrl.getAllAokKnowledgebaseAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aokKnowledgebaseAudit.rawAttributes,req.body);
        ...
        if (objAokKnowledgebaseAudit && objAokKnowledgebaseAudit.rows && objAokKnowledgebaseAudit.count) {
            util.setSuccess(200, 'AokKnowledgebaseAudit retrieved', objAokKnowledgebaseAudit.rows, objAokKnowledgebaseAudit.count, req.query.limit, req.query.offset);
        } else {
