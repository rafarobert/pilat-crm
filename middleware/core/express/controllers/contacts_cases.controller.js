...
contactsCasesCtrl.getAllContactsCases = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.contactsCases.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objContactsCases = await contactCaseService.getAllContactsCases(req.query);
        if (objContactsCases && objContactsCases.rows && objContactsCases.count) {
            util.setSuccess(200, 'ContactsCases retrieved', objContactsCases.rows, objContactsCases.count, req.query.limit, req.query.offset);
        } else {
