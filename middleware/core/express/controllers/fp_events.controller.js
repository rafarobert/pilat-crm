...
fpEventsCtrl.getAllFpEvents = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.fpEvents.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objFpEvents = await fpEventService.getAllFpEvents(req.query);
        if (objFpEvents && objFpEvents.rows && objFpEvents.count) {
            util.setSuccess(200, 'FpEvents retrieved', objFpEvents.rows, objFpEvents.count, req.query.limit, req.query.offset);
        } else {
