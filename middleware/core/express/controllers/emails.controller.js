...
emailsCtrl.getAllEmails = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emails.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmails = await emailService.getAllEmails(req.query);
        if (objEmails && objEmails.rows && objEmails.count) {
            util.setSuccess(200, 'Emails retrieved', objEmails.rows, objEmails.count, req.query.limit, req.query.offset);
        } else {
