...
callsUsersCtrl.getAllCallsUsers = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.callsUsers.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCallsUsers = await callUserService.getAllCallsUsers(req.query);
        if (objCallsUsers && objCallsUsers.rows && objCallsUsers.count) {
            util.setSuccess(200, 'CallsUsers retrieved', objCallsUsers.rows, objCallsUsers.count, req.query.limit, req.query.offset);
        } else {
