...
remindersInviteesCtrl.getAllRemindersInvitees = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.remindersInvitees.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objRemindersInvitees = await reminderInviteeService.getAllRemindersInvitees(req.query);
        if (objRemindersInvitees && objRemindersInvitees.rows && objRemindersInvitees.count) {
            util.setSuccess(200, 'RemindersInvitees retrieved', objRemindersInvitees.rows, objRemindersInvitees.count, req.query.limit, req.query.offset);
        } else {
