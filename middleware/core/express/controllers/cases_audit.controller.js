...
casesAuditCtrl.getAllCasesAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.casesAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCasesAudit = await caseAuditService.getAllCasesAudit(req.query);
        if (objCasesAudit && objCasesAudit.rows && objCasesAudit.count) {
            util.setSuccess(200, 'CasesAudit retrieved', objCasesAudit.rows, objCasesAudit.count, req.query.limit, req.query.offset);
        } else {
