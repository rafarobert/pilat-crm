...
oauthTokensCtrl.getAllOauthTokens = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.oauthTokens.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objOauthTokens = await oauthTokenService.getAllOauthTokens(req.query);
        if (objOauthTokens && objOauthTokens.rows && objOauthTokens.count) {
            util.setSuccess(200, 'OauthTokens retrieved', objOauthTokens.rows, objOauthTokens.count, req.query.limit, req.query.offset);
        } else {
