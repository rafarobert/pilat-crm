...
usersLastImportCtrl.getAllUsersLastImport = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.usersLastImport.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objUsersLastImport = await userLastImportService.getAllUsersLastImport(req.query);
        if (objUsersLastImport && objUsersLastImport.rows && objUsersLastImport.count) {
            util.setSuccess(200, 'UsersLastImport retrieved', objUsersLastImport.rows, objUsersLastImport.count, req.query.limit, req.query.offset);
        } else {
