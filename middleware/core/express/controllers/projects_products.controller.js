...
projectsProductsCtrl.getAllProjectsProducts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.projectsProducts.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objProjectsProducts = await projectProductService.getAllProjectsProducts(req.query);
        if (objProjectsProducts && objProjectsProducts.rows && objProjectsProducts.count) {
            util.setSuccess(200, 'ProjectsProducts retrieved', objProjectsProducts.rows, objProjectsProducts.count, req.query.limit, req.query.offset);
        } else {
