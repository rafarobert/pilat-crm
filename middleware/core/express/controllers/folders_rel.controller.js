...
foldersRelCtrl.getAllFoldersRel = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.foldersRel.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objFoldersRel = await folderRelService.getAllFoldersRel(req.query);
        if (objFoldersRel && objFoldersRel.rows && objFoldersRel.count) {
            util.setSuccess(200, 'FoldersRel retrieved', objFoldersRel.rows, objFoldersRel.count, req.query.limit, req.query.offset);
        } else {
