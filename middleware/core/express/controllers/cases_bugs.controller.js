...
casesBugsCtrl.getAllCasesBugs = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.casesBugs.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCasesBugs = await caseBugService.getAllCasesBugs(req.query);
        if (objCasesBugs && objCasesBugs.rows && objCasesBugs.count) {
            util.setSuccess(200, 'CasesBugs retrieved', objCasesBugs.rows, objCasesBugs.count, req.query.limit, req.query.offset);
        } else {
