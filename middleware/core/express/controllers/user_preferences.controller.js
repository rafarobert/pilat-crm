...
userPreferencesCtrl.getAllUserPreferences = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.userPreferences.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objUserPreferences = await userPreferenceService.getAllUserPreferences(req.query);
        if (objUserPreferences && objUserPreferences.rows && objUserPreferences.count) {
            util.setSuccess(200, 'UserPreferences retrieved', objUserPreferences.rows, objUserPreferences.count, req.query.limit, req.query.offset);
        } else {
