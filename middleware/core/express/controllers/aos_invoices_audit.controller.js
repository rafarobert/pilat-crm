...
aosInvoicesAuditCtrl.getAllAosInvoicesAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosInvoicesAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosInvoicesAudit = await aoInvoiceAuditService.getAllAosInvoicesAudit(req.query);
        if (objAosInvoicesAudit && objAosInvoicesAudit.rows && objAosInvoicesAudit.count) {
            util.setSuccess(200, 'AosInvoicesAudit retrieved', objAosInvoicesAudit.rows, objAosInvoicesAudit.count, req.query.limit, req.query.offset);
        } else {
