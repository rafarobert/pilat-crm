...
campaignTrkrsCtrl.getAllCampaignTrkrs = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.campaignTrkrs.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCampaignTrkrs = await campaignTrkrService.getAllCampaignTrkrs(req.query);
        if (objCampaignTrkrs && objCampaignTrkrs.rows && objCampaignTrkrs.count) {
            util.setSuccess(200, 'CampaignTrkrs retrieved', objCampaignTrkrs.rows, objCampaignTrkrs.count, req.query.limit, req.query.offset);
        } else {
