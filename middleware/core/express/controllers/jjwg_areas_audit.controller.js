...
jjwgAreasAuditCtrl.getAllJjwgAreasAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.jjwgAreasAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objJjwgAreasAudit = await jjwgAreaAuditService.getAllJjwgAreasAudit(req.query);
        if (objJjwgAreasAudit && objJjwgAreasAudit.rows && objJjwgAreasAudit.count) {
            util.setSuccess(200, 'JjwgAreasAudit retrieved', objJjwgAreasAudit.rows, objJjwgAreasAudit.count, req.query.limit, req.query.offset);
        } else {
