...
aodIndexeventCtrl.getAllAodIndexevent = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aodIndexevent.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAodIndexevent = await aodIndexeventService.getAllAodIndexevent(req.query);
        if (objAodIndexevent && objAodIndexevent.rows && objAodIndexevent.count) {
            util.setSuccess(200, 'AodIndexevent retrieved', objAodIndexevent.rows, objAodIndexevent.count, req.query.limit, req.query.offset);
        } else {
