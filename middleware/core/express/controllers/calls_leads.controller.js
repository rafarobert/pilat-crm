...
callsLeadsCtrl.getAllCallsLeads = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.callsLeads.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCallsLeads = await callLeadService.getAllCallsLeads(req.query);
        if (objCallsLeads && objCallsLeads.rows && objCallsLeads.count) {
            util.setSuccess(200, 'CallsLeads retrieved', objCallsLeads.rows, objCallsLeads.count, req.query.limit, req.query.offset);
        } else {
