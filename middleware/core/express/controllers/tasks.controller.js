...
tasksCtrl.getAllTasks = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.tasks.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objTasks = await taskService.getAllTasks(req.query);
        if (objTasks && objTasks.rows && objTasks.count) {
            util.setSuccess(200, 'Tasks retrieved', objTasks.rows, objTasks.count, req.query.limit, req.query.offset);
        } else {
