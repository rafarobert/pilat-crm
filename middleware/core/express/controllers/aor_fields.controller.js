...
aorFieldsCtrl.getAllAorFields = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aorFields.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAorFields = await aorFieldService.getAllAorFields(req.query);
        if (objAorFields && objAorFields.rows && objAorFields.count) {
            util.setSuccess(200, 'AorFields retrieved', objAorFields.rows, objAorFields.count, req.query.limit, req.query.offset);
        } else {
