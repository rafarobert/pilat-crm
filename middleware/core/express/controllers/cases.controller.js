...
casesCtrl.getAllCases = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.cases.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCases = await caseService.getAllCases(req.query);
        if (objCases && objCases.rows && objCases.count) {
            util.setSuccess(200, 'Cases retrieved', objCases.rows, objCases.count, req.query.limit, req.query.offset);
        } else {
