...
currenciesCtrl.getAllCurrencies = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.currencies.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCurrencies = await currencyService.getAllCurrencies(req.query);
        if (objCurrencies && objCurrencies.rows && objCurrencies.count) {
            util.setSuccess(200, 'Currencies retrieved', objCurrencies.rows, objCurrencies.count, req.query.limit, req.query.offset);
        } else {
