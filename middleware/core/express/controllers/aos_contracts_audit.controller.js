...
aosContractsAuditCtrl.getAllAosContractsAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosContractsAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosContractsAudit = await aoContractAuditService.getAllAosContractsAudit(req.query);
        if (objAosContractsAudit && objAosContractsAudit.rows && objAosContractsAudit.count) {
            util.setSuccess(200, 'AosContractsAudit retrieved', objAosContractsAudit.rows, objAosContractsAudit.count, req.query.limit, req.query.offset);
        } else {
