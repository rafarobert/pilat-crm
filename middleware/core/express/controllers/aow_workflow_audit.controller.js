...
aowWorkflowAuditCtrl.getAllAowWorkflowAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aowWorkflowAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAowWorkflowAudit = await aowWorkflowAuditService.getAllAowWorkflowAudit(req.query);
        if (objAowWorkflowAudit && objAowWorkflowAudit.rows && objAowWorkflowAudit.count) {
            util.setSuccess(200, 'AowWorkflowAudit retrieved', objAowWorkflowAudit.rows, objAowWorkflowAudit.count, req.query.limit, req.query.offset);
        } else {
