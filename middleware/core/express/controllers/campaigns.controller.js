...
campaignsCtrl.getAllCampaigns = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.campaigns.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCampaigns = await campaignService.getAllCampaigns(req.query);
        if (objCampaigns && objCampaigns.rows && objCampaigns.count) {
            util.setSuccess(200, 'Campaigns retrieved', objCampaigns.rows, objCampaigns.count, req.query.limit, req.query.offset);
        } else {
