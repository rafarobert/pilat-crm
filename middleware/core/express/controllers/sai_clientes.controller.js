...
saiClientesCtrl.getAllSaiClientes = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.saiClientes.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objSaiClientes = await saiClienteService.getAllSaiClientes(req.query);
        if (objSaiClientes && objSaiClientes.rows && objSaiClientes.count) {
            util.setSuccess(200, 'SaiClientes retrieved', objSaiClientes.rows, objSaiClientes.count, req.query.limit, req.query.offset);
        } else {
