...
releasesCtrl.getAllReleases = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.releases.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objReleases = await releaseService.getAllReleases(req.query);
        if (objReleases && objReleases.rows && objReleases.count) {
            util.setSuccess(200, 'Releases retrieved', objReleases.rows, objReleases.count, req.query.limit, req.query.offset);
        } else {
