...
aowActionsCtrl.getAllAowActions = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aowActions.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAowActions = await aowActionService.getAllAowActions(req.query);
        if (objAowActions && objAowActions.rows && objAowActions.count) {
            util.setSuccess(200, 'AowActions retrieved', objAowActions.rows, objAowActions.count, req.query.limit, req.query.offset);
        } else {
