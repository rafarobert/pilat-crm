...
aowConditionsCtrl.getAllAowConditions = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aowConditions.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAowConditions = await aowConditionService.getAllAowConditions(req.query);
        if (objAowConditions && objAowConditions.rows && objAowConditions.count) {
            util.setSuccess(200, 'AowConditions retrieved', objAowConditions.rows, objAowConditions.count, req.query.limit, req.query.offset);
        } else {
