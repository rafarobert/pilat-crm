...
inboundEmailCacheTsCtrl.getAllInboundEmailCacheTs = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.inboundEmailCacheTs.rawAttributes,req.body);
        ...
        if (objInboundEmailCacheTs && objInboundEmailCacheTs.rows && objInboundEmailCacheTs.count) {
            util.setSuccess(200, 'InboundEmailCacheTs retrieved', objInboundEmailCacheTs.rows, objInboundEmailCacheTs.count, req.query.limit, req.query.offset);
        } else {
