...
aosInvoicesCtrl.getAllAosInvoices = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosInvoices.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosInvoices = await aoInvoiceService.getAllAosInvoices(req.query);
        if (objAosInvoices && objAosInvoices.rows && objAosInvoices.count) {
            util.setSuccess(200, 'AosInvoices retrieved', objAosInvoices.rows, objAosInvoices.count, req.query.limit, req.query.offset);
        } else {
