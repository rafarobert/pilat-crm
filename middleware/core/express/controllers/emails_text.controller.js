...
emailsTextCtrl.getAllEmailsText = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emailsText.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmailsText = await emailTextService.getAllEmailsText(req.query);
        if (objEmailsText && objEmailsText.rows && objEmailsText.count) {
            util.setSuccess(200, 'EmailsText retrieved', objEmailsText.rows, objEmailsText.count, req.query.limit, req.query.offset);
        } else {
