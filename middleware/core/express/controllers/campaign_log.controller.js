...
campaignLogCtrl.getAllCampaignLog = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.campaignLog.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objCampaignLog = await campaignLogService.getAllCampaignLog(req.query);
        if (objCampaignLog && objCampaignLog.rows && objCampaignLog.count) {
            util.setSuccess(200, 'CampaignLog retrieved', objCampaignLog.rows, objCampaignLog.count, req.query.limit, req.query.offset);
        } else {
