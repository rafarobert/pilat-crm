...
jjwgAddressCacheCtrl.getAllJjwgAddressCache = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.jjwgAddressCache.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objJjwgAddressCache = await jjwgAddresCacheService.getAllJjwgAddressCache(req.query);
        if (objJjwgAddressCache && objJjwgAddressCache.rows && objJjwgAddressCache.count) {
            util.setSuccess(200, 'JjwgAddressCache retrieved', objJjwgAddressCache.rows, objJjwgAddressCache.count, req.query.limit, req.query.offset);
        } else {
