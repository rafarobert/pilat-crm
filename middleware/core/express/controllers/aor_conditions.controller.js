...
aorConditionsCtrl.getAllAorConditions = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aorConditions.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAorConditions = await aorConditionService.getAllAorConditions(req.query);
        if (objAorConditions && objAorConditions.rows && objAorConditions.count) {
            util.setSuccess(200, 'AorConditions retrieved', objAorConditions.rows, objAorConditions.count, req.query.limit, req.query.offset);
        } else {
