...
aosProductsCtrl.getAllAosProducts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosProducts.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosProducts = await aoProductService.getAllAosProducts(req.query);
        if (objAosProducts && objAosProducts.rows && objAosProducts.count) {
            util.setSuccess(200, 'AosProducts retrieved', objAosProducts.rows, objAosProducts.count, req.query.limit, req.query.offset);
        } else {
