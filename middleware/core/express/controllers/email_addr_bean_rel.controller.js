...
emailAddrBeanRelCtrl.getAllEmailAddrBeanRel = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emailAddrBeanRel.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmailAddrBeanRel = await emailAddrBeanRelService.getAllEmailAddrBeanRel(req.query);
        if (objEmailAddrBeanRel && objEmailAddrBeanRel.rows && objEmailAddrBeanRel.count) {
            util.setSuccess(200, 'EmailAddrBeanRel retrieved', objEmailAddrBeanRel.rows, objEmailAddrBeanRel.count, req.query.limit, req.query.offset);
        } else {
