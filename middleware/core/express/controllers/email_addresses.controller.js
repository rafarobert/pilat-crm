...
emailAddressesCtrl.getAllEmailAddresses = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emailAddresses.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmailAddresses = await emailAddresseService.getAllEmailAddresses(req.query);
        if (objEmailAddresses && objEmailAddresses.rows && objEmailAddresses.count) {
            util.setSuccess(200, 'EmailAddresses retrieved', objEmailAddresses.rows, objEmailAddresses.count, req.query.limit, req.query.offset);
        } else {
