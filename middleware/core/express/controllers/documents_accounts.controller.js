...
documentsAccountsCtrl.getAllDocumentsAccounts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.documentsAccounts.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objDocumentsAccounts = await documentAccountService.getAllDocumentsAccounts(req.query);
        if (objDocumentsAccounts && objDocumentsAccounts.rows && objDocumentsAccounts.count) {
            util.setSuccess(200, 'DocumentsAccounts retrieved', objDocumentsAccounts.rows, objDocumentsAccounts.count, req.query.limit, req.query.offset);
        } else {
