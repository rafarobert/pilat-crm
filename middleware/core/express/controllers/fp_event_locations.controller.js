...
fpEventLocationsCtrl.getAllFpEventLocations = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.fpEventLocations.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objFpEventLocations = await fpEventLocationService.getAllFpEventLocations(req.query);
        if (objFpEventLocations && objFpEventLocations.rows && objFpEventLocations.count) {
            util.setSuccess(200, 'FpEventLocations retrieved', objFpEventLocations.rows, objFpEventLocations.count, req.query.limit, req.query.offset);
        } else {
