...
accountsBugsCtrl.getAllAccountsBugs = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.accountsBugs.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAccountsBugs = await accountBugService.getAllAccountsBugs(req.query);
        if (objAccountsBugs && objAccountsBugs.rows && objAccountsBugs.count) {
            util.setSuccess(200, 'AccountsBugs retrieved', objAccountsBugs.rows, objAccountsBugs.count, req.query.limit, req.query.offset);
        } else {
