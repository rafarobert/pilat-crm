...
aosLineItemGroupsCtrl.getAllAosLineItemGroups = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosLineItemGroups.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosLineItemGroups = await aoLineItemGroupService.getAllAosLineItemGroups(req.query);
        if (objAosLineItemGroups && objAosLineItemGroups.rows && objAosLineItemGroups.count) {
            util.setSuccess(200, 'AosLineItemGroups retrieved', objAosLineItemGroups.rows, objAosLineItemGroups.count, req.query.limit, req.query.offset);
        } else {
