...
prospectsCtrl.getAllProspects = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.prospects.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objProspects = await prospectService.getAllProspects(req.query);
        if (objProspects && objProspects.rows && objProspects.count) {
            util.setSuccess(200, 'Prospects retrieved', objProspects.rows, objProspects.count, req.query.limit, req.query.offset);
        } else {
