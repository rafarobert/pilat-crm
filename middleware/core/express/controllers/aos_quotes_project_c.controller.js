...
aosQuotesProjectCCtrl.getAllAosQuotesProjectC = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosQuotesProjectC.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosQuotesProjectC = await aoQuoteProjectCService.getAllAosQuotesProjectC(req.query);
        if (objAosQuotesProjectC && objAosQuotesProjectC.rows && objAosQuotesProjectC.count) {
            util.setSuccess(200, 'AosQuotesProjectC retrieved', objAosQuotesProjectC.rows, objAosQuotesProjectC.count, req.query.limit, req.query.offset);
        } else {
