...
usersCstmCtrl.getAllUsersCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.usersCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objUsersCstm = await userCstmService.getAllUsersCstm(req.query);
        if (objUsersCstm && objUsersCstm.rows && objUsersCstm.count) {
            util.setSuccess(200, 'UsersCstm retrieved', objUsersCstm.rows, objUsersCstm.count, req.query.limit, req.query.offset);
        } else {
