...
aodIndexCtrl.getAllAodIndex = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aodIndex.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAodIndex = await aodIndexService.getAllAodIndex(req.query);
        if (objAodIndex && objAodIndex.rows && objAodIndex.count) {
            util.setSuccess(200, 'AodIndex retrieved', objAodIndex.rows, objAodIndex.count, req.query.limit, req.query.offset);
        } else {
