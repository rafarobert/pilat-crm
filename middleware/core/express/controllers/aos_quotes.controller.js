...
aosQuotesCtrl.getAllAosQuotes = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosQuotes.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosQuotes = await aoQuoteService.getAllAosQuotes(req.query);
        if (objAosQuotes && objAosQuotes.rows && objAosQuotes.count) {
            util.setSuccess(200, 'AosQuotes retrieved', objAosQuotes.rows, objAosQuotes.count, req.query.limit, req.query.offset);
        } else {
