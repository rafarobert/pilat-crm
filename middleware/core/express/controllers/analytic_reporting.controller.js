...
analyticReportingCtrl.getAllAnalyticReporting = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.analyticReporting.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAnalyticReporting = await analyticReportingService.getAllAnalyticReporting(req.query);
        if (objAnalyticReporting && objAnalyticReporting.rows && objAnalyticReporting.count) {
            util.setSuccess(200, 'AnalyticReporting retrieved', objAnalyticReporting.rows, objAnalyticReporting.count, req.query.limit, req.query.offset);
        } else {
