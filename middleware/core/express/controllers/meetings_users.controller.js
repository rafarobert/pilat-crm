...
meetingsUsersCtrl.getAllMeetingsUsers = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.meetingsUsers.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objMeetingsUsers = await meetingUserService.getAllMeetingsUsers(req.query);
        if (objMeetingsUsers && objMeetingsUsers.rows && objMeetingsUsers.count) {
            util.setSuccess(200, 'MeetingsUsers retrieved', objMeetingsUsers.rows, objMeetingsUsers.count, req.query.limit, req.query.offset);
        } else {
