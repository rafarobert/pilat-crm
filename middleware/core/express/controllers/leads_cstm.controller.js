...
leadsCstmCtrl.getAllLeadsCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.leadsCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objLeadsCstm = await leadCstmService.getAllLeadsCstm(req.query);
        if (objLeadsCstm && objLeadsCstm.rows && objLeadsCstm.count) {
            util.setSuccess(200, 'LeadsCstm retrieved', objLeadsCstm.rows, objLeadsCstm.count, req.query.limit, req.query.offset);
        } else {
