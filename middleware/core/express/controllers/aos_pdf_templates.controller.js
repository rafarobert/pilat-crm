...
aosPdfTemplatesCtrl.getAllAosPdfTemplates = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosPdfTemplates.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAosPdfTemplates = await aoPdfTemplateService.getAllAosPdfTemplates(req.query);
        if (objAosPdfTemplates && objAosPdfTemplates.rows && objAosPdfTemplates.count) {
            util.setSuccess(200, 'AosPdfTemplates retrieved', objAosPdfTemplates.rows, objAosPdfTemplates.count, req.query.limit, req.query.offset);
        } else {
