...
favoritesCtrl.getAllFavorites = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.favorites.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objFavorites = await favoriteService.getAllFavorites(req.query);
        if (objFavorites && objFavorites.rows && objFavorites.count) {
            util.setSuccess(200, 'Favorites retrieved', objFavorites.rows, objFavorites.count, req.query.limit, req.query.offset);
        } else {
