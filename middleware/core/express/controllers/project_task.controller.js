...
projectTaskCtrl.getAllProjectTask = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.projectTask.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objProjectTask = await projectTaskService.getAllProjectTask(req.query);
        if (objProjectTask && objProjectTask.rows && objProjectTask.count) {
            util.setSuccess(200, 'ProjectTask retrieved', objProjectTask.rows, objProjectTask.count, req.query.limit, req.query.offset);
        } else {
