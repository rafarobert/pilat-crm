...
accountsCasesCtrl.getAllAccountsCases = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.accountsCases.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAccountsCases = await accountCaseService.getAllAccountsCases(req.query);
        if (objAccountsCases && objAccountsCases.rows && objAccountsCases.count) {
            util.setSuccess(200, 'AccountsCases retrieved', objAccountsCases.rows, objAccountsCases.count, req.query.limit, req.query.offset);
        } else {
