...
opportunitiesContactsCtrl.getAllOpportunitiesContacts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.opportunitiesContacts.rawAttributes,req.body);
        ...
        if (objOpportunitiesContacts && objOpportunitiesContacts.rows && objOpportunitiesContacts.count) {
            util.setSuccess(200, 'OpportunitiesContacts retrieved', objOpportunitiesContacts.rows, objOpportunitiesContacts.count, req.query.limit, req.query.offset);
        } else {
