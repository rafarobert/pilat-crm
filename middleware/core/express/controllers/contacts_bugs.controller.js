...
contactsBugsCtrl.getAllContactsBugs = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.contactsBugs.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objContactsBugs = await contactBugService.getAllContactsBugs(req.query);
        if (objContactsBugs && objContactsBugs.rows && objContactsBugs.count) {
            util.setSuccess(200, 'ContactsBugs retrieved', objContactsBugs.rows, objContactsBugs.count, req.query.limit, req.query.offset);
        } else {
