...
inboundEmailCtrl.getAllInboundEmail = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.inboundEmail.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objInboundEmail = await inboundEmailService.getAllInboundEmail(req.query);
        if (objInboundEmail && objInboundEmail.rows && objInboundEmail.count) {
            util.setSuccess(200, 'InboundEmail retrieved', objInboundEmail.rows, objInboundEmail.count, req.query.limit, req.query.offset);
        } else {
