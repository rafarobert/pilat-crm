...
contactsUsersCtrl.getAllContactsUsers = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.contactsUsers.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objContactsUsers = await contactUserService.getAllContactsUsers(req.query);
        if (objContactsUsers && objContactsUsers.rows && objContactsUsers.count) {
            util.setSuccess(200, 'ContactsUsers retrieved', objContactsUsers.rows, objContactsUsers.count, req.query.limit, req.query.offset);
        } else {
