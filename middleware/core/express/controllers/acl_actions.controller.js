...
aclActionsCtrl.getAllAclActions = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aclActions.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAclActions = await aclActionService.getAllAclActions(req.query);
        if (objAclActions && objAclActions.rows && objAclActions.count) {
            util.setSuccess(200, 'AclActions retrieved', objAclActions.rows, objAclActions.count, req.query.limit, req.query.offset);
        } else {
