...
projectsOpportunitiesCtrl.getAllProjectsOpportunities = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.projectsOpportunities.rawAttributes,req.body);
        ...
        if (objProjectsOpportunities && objProjectsOpportunities.rows && objProjectsOpportunities.count) {
            util.setSuccess(200, 'ProjectsOpportunities retrieved', objProjectsOpportunities.rows, objProjectsOpportunities.count, req.query.limit, req.query.offset);
        } else {
