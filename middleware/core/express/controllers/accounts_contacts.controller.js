...
accountsContactsCtrl.getAllAccountsContacts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.accountsContacts.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAccountsContacts = await accountContactService.getAllAccountsContacts(req.query);
        if (objAccountsContacts && objAccountsContacts.rows && objAccountsContacts.count) {
            util.setSuccess(200, 'AccountsContacts retrieved', objAccountsContacts.rows, objAccountsContacts.count, req.query.limit, req.query.offset);
        } else {
