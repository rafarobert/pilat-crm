...
surveyquestionsCtrl.getAllSurveyquestions = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.surveyquestions.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objSurveyquestions = await surveyquestionService.getAllSurveyquestions(req.query);
        if (objSurveyquestions && objSurveyquestions.rows && objSurveyquestions.count) {
            util.setSuccess(200, 'Surveyquestions retrieved', objSurveyquestions.rows, objSurveyquestions.count, req.query.limit, req.query.offset);
        } else {
