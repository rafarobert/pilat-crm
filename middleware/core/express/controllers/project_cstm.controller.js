...
projectCstmCtrl.getAllProjectCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.projectCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objProjectCstm = await projectCstmService.getAllProjectCstm(req.query);
        if (objProjectCstm && objProjectCstm.rows && objProjectCstm.count) {
            util.setSuccess(200, 'ProjectCstm retrieved', objProjectCstm.rows, objProjectCstm.count, req.query.limit, req.query.offset);
        } else {
