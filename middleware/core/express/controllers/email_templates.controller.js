...
emailTemplatesCtrl.getAllEmailTemplates = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.emailTemplates.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objEmailTemplates = await emailTemplateService.getAllEmailTemplates(req.query);
        if (objEmailTemplates && objEmailTemplates.rows && objEmailTemplates.count) {
            util.setSuccess(200, 'EmailTemplates retrieved', objEmailTemplates.rows, objEmailTemplates.count, req.query.limit, req.query.offset);
        } else {
