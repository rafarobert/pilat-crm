...
alertsCtrl.getAllAlerts = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.alerts.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAlerts = await alertService.getAllAlerts(req.query);
        if (objAlerts && objAlerts.rows && objAlerts.count) {
            util.setSuccess(200, 'Alerts retrieved', objAlerts.rows, objAlerts.count, req.query.limit, req.query.offset);
        } else {
