...
spotsCtrl.getAllSpots = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.spots.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objSpots = await spotService.getAllSpots(req.query);
        if (objSpots && objSpots.rows && objSpots.count) {
            util.setSuccess(200, 'Spots retrieved', objSpots.rows, objSpots.count, req.query.limit, req.query.offset);
        } else {
