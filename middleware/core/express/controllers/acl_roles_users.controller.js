...
aclRolesUsersCtrl.getAllAclRolesUsers = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aclRolesUsers.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objAclRolesUsers = await aclRoleUserService.getAllAclRolesUsers(req.query);
        if (objAclRolesUsers && objAclRolesUsers.rows && objAclRolesUsers.count) {
            util.setSuccess(200, 'AclRolesUsers retrieved', objAclRolesUsers.rows, objAclRolesUsers.count, req.query.limit, req.query.offset);
        } else {
