...
oauth2tokensCtrl.getAllOauth2tokens = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.oauth2tokens.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objOauth2tokens = await oauth2tokenService.getAllOauth2tokens(req.query);
        if (objOauth2tokens && objOauth2tokens.rows && objOauth2tokens.count) {
            util.setSuccess(200, 'Oauth2tokens retrieved', objOauth2tokens.rows, objOauth2tokens.count, req.query.limit, req.query.offset);
        } else {
