...
contactsCstmCtrl.getAllContactsCstm = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.contactsCstm.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objContactsCstm = await contactCstmService.getAllContactsCstm(req.query);
        if (objContactsCstm && objContactsCstm.rows && objContactsCstm.count) {
            util.setSuccess(200, 'ContactsCstm retrieved', objContactsCstm.rows, objContactsCstm.count, req.query.limit, req.query.offset);
        } else {
