...
jjwgMarkersCtrl.getAllJjwgMarkers = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.jjwgMarkers.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objJjwgMarkers = await jjwgMarkerService.getAllJjwgMarkers(req.query);
        if (objJjwgMarkers && objJjwgMarkers.rows && objJjwgMarkers.count) {
            util.setSuccess(200, 'JjwgMarkers retrieved', objJjwgMarkers.rows, objJjwgMarkers.count, req.query.limit, req.query.offset);
        } else {
