...
aosQuotesAosInvoicesCCtrl.getAllAosQuotesAosInvoicesC = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.aosQuotesAosInvoicesC.rawAttributes,req.body);
        ...
        if (objAosQuotesAosInvoicesC && objAosQuotesAosInvoicesC.rows && objAosQuotesAosInvoicesC.count) {
            util.setSuccess(200, 'AosQuotesAosInvoicesC retrieved', objAosQuotesAosInvoicesC.rows, objAosQuotesAosInvoicesC.count, req.query.limit, req.query.offset);
        } else {
