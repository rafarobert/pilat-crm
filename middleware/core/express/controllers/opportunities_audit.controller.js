...
opportunitiesAuditCtrl.getAllOpportunitiesAudit = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.opportunitiesAudit.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objOpportunitiesAudit = await opportunityAuditService.getAllOpportunitiesAudit(req.query);
        if (objOpportunitiesAudit && objOpportunitiesAudit.rows && objOpportunitiesAudit.count) {
            util.setSuccess(200, 'OpportunitiesAudit retrieved', objOpportunitiesAudit.rows, objOpportunitiesAudit.count, req.query.limit, req.query.offset);
        } else {
