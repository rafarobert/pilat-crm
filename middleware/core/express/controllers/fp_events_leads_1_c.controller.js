...
fpEventsLeads1CCtrl.getAllFpEventsLeads1C = async (req, res) => {
    try {
        ...
        const [column, dir] = util.getOrderByColumnDirection(models.sequelize.fpEventsLeads1C.rawAttributes,req.body);
        req.query.limit = length ? length : req.query.limit;
        req.query.offset = start ? start : req.query.offset;
        req.query.order = column && dir ? [[column,dir]] : req.query.order;

        const objFpEventsLeads1C = await fpEventLead1CService.getAllFpEventsLeads1C(req.query);
        if (objFpEventsLeads1C && objFpEventsLeads1C.rows && objFpEventsLeads1C.count) {
            util.setSuccess(200, 'FpEventsLeads1C retrieved', objFpEventsLeads1C.rows, objFpEventsLeads1C.count, req.query.limit, req.query.offset);
        } else {
