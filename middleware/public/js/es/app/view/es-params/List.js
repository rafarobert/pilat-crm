...
Ext.define('es.view.es-params.List', {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esParams').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esParams').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esParams').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esParams').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsDictionariesParDictionaryWithDicCode.data.items;
                	let label;
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
