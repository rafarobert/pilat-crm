...
Ext.define('es.view.es-object-attribute-instances.List', {
	...
	//<es-section>
	store: 'esObjectAttributeInstances',
	//</es-section>
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esObjectAttributeInstances').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esObjectAttributeInstances').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esObjectAttributeInstances').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esObjectAttributeInstances').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'obj_att_ins_group',
				dataIndex: 'obj_att_ins_group',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'obj_att_id',
				dataIndex: 'obj_att_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsObjectAttributesObjAttWithObjAttDefaultValue.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.obj_att_default_value;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'obj_att_ins_par_status_id',
				dataIndex: 'obj_att_ins_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsObjAttInsParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
