...
Ext.define('es.view.es-person-attributes.List', {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esPersonAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esPersonAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esPersonAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esPersonAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsPeoplePerWithPerFirstName.data.items;
                	let label;
			...
			{
				text: 'per_att_par_status_id',
				dataIndex: 'per_att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsPerAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
