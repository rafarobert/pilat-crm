...
Ext.define('es.view.es-mails.List', {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esMails').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esMails').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esMails').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esMails').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsUsersMaiUserAccountWithUsrUsername.data.items;
                	let label;
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsBusinessMaiBusWithBusName.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.bus_name;
                		}
                	}
                	return label;
                },
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsMaiParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
