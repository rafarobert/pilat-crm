...
var storeEsParamsCroParStatusWithParCod = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-crons/findEsParamsCroParStatusWithParCod',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersCreatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-crons/findEsUsersCreatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersUpdatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-crons/findEsUsersUpdatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
Ext.define('es.view.es-crons.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esCrons').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esCrons').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esCrons').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esCrons').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'cro_par_status_id',
				dataIndex: 'cro_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsCroParStatusWithParCod.data.items;
                	let label;
				...
				editor:  new Ext.form.field.ComboBox({
					...
					triggerAction: 'all',
					selectOnTab: true,
					store: storeEsParamsCroParStatusWithParCod,
					valueField: "_id",
