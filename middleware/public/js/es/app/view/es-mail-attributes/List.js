...
Ext.define('es.view.es-mail-attributes.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esMailAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esMailAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esMailAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esMailAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'mai_att_description',
				dataIndex: 'mai_att_description',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsMailsMaiWithMaiDescription.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.mai_description;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'mai_att_par_status_id',
				dataIndex: 'mai_att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsMaiAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
