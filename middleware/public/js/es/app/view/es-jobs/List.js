...
var storeEsParamsJobParStatusWithParCod = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-jobs/findEsParamsJobParStatusWithParCod',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersCreatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-jobs/findEsUsersCreatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersUpdatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-jobs/findEsUsersUpdatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
Ext.define('es.view.es-jobs.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esJobs').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esJobs').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esJobs').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esJobs').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'job_par_status_id',
				dataIndex: 'job_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsJobParStatusWithParCod.data.items;
                	let label;
				...
				editor:  new Ext.form.field.ComboBox({
					...
					triggerAction: 'all',
					selectOnTab: true,
					store: storeEsParamsJobParStatusWithParCod,
					valueField: "_id",
