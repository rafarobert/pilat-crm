...
Ext.define('es.view.es-logs.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esLogs').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esLogs').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esLogs').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esLogs').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'log_obj_id',
				dataIndex: 'log_obj_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsObjectsLogObjWithObjDescription.data.items;
                	let label;
			...
			{
				text: 'log_par_status_id',
				dataIndex: 'log_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsLogParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
