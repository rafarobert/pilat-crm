...
var storeEsProfilesProWithProCode = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profile-pages/findEsProfilesProWithProCode',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsPagesPagWithPagCode = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profile-pages/findEsPagesPagWithPagCode',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsParamsProPagParStatusWithParCod = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profile-pages/findEsParamsProPagParStatusWithParCod',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersCreatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profile-pages/findEsUsersCreatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersUpdatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profile-pages/findEsUsersUpdatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
Ext.define('es.view.es-profile-pages.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esProfilePages').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esProfilePages').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esProfilePages').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esProfilePages').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			
			{
				...
				dataIndex: 'pag_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsPagesPagWithPagCode.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
			...
			{
				text: 'pro_pag_par_status_id',
				dataIndex: 'pro_pag_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsProPagParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
				...
				editor:  new Ext.form.field.ComboBox({
					...
					triggerAction: 'all',
					selectOnTab: true,
					store: storeEsParamsProPagParStatusWithParCod,
					valueField: "_id",
