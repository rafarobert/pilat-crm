...
var storeEsParamsProParStatusWithParCod = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profiles/findEsParamsProParStatusWithParCod',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersCreatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profiles/findEsUsersCreatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersUpdatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-profiles/findEsUsersUpdatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
Ext.define('es.view.es-profiles.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esProfiles').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esProfiles').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esProfiles').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esProfiles').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'pro_par_status_id',
				dataIndex: 'pro_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsProParStatusWithParCod.data.items;
                	let label;
				...
				editor:  new Ext.form.field.ComboBox({
					...
					triggerAction: 'all',
					selectOnTab: true,
					store: storeEsParamsProParStatusWithParCod,
					valueField: "_id",
