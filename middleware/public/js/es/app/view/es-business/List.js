...
Ext.define('es.view.es-business.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esBusiness').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esBusiness').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esBusiness').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esBusiness').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'bus_description',
				dataIndex: 'bus_description',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'bus_user_account',
				dataIndex: 'bus_user_account',
				renderer: (val, metaData, r) => {
                	let items = storeEsUsersBusUserAccountWithUsrUsername.data.items;
                	let label;
			...
			{
				text: 'bus_par_status_id',
				dataIndex: 'bus_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsBusParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
