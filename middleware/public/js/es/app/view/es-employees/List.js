...
var storeEsParamsEmpParStatusWithParCod = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-employees/findEsParamsEmpParStatusWithParCod',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersCreatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-employees/findEsUsersCreatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
var storeEsUsersUpdatedByWithUsrUsername = new Ext.data.JsonStore({
	proxy: {
		type: 'ajax',
		url : '/api-esvender/es-employees/findEsUsersUpdatedByWithUsrUsername',
		reader: {
			type: 'json',
			root: 'data'
		},
...
Ext.define('es.view.es-employees.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esEmployees').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esEmployees').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esEmployees').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esEmployees').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'emp_par_status_id',
				dataIndex: 'emp_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsEmpParStatusWithParCod.data.items;
                	let label;
				...
				editor:  new Ext.form.field.ComboBox({
					...
					triggerAction: 'all',
					selectOnTab: true,
					store: storeEsParamsEmpParStatusWithParCod,
					valueField: "_id",
