...
Ext.define('es.view.es-log-attribute-instances.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esLogAttributeInstances').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esLogAttributeInstances').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esLogAttributeInstances').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esLogAttributeInstances').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'log_att_ins_group',
				dataIndex: 'log_att_ins_group',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'log_att_id',
				dataIndex: 'log_att_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsLogAttributesLogAttWithLogAttName.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.log_att_name;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'log_att_ins_par_status_id',
				dataIndex: 'log_att_ins_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsLogAttInsParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
