...
Ext.define('es.view.es-attributes.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'att_par_type_id',
				dataIndex: 'att_par_type_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsAttParTypeWithParCod.data.items;
                	let label;
			...
			{
				text: 'att_par_status_id',
				dataIndex: 'att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
