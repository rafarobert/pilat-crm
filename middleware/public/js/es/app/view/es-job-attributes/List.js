...
Ext.define('es.view.es-job-attributes.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esJobAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esJobAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esJobAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esJobAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'job_att_description',
				dataIndex: 'job_att_description',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsJobsJobWithJobDescription.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.job_description;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'job_att_par_status_id',
				dataIndex: 'job_att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsJobAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
