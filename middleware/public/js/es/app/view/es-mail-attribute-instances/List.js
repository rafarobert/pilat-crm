...
Ext.define('es.view.es-mail-attribute-instances.List', {
	...
	//<es-section>
	store: 'esMailAttributeInstances',
	//</es-section>
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esMailAttributeInstances').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esMailAttributeInstances').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esMailAttributeInstances').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esMailAttributeInstances').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'mai_att_ins_group',
				dataIndex: 'mai_att_ins_group',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'mai_att_id',
				dataIndex: 'mai_att_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsMailAttributesMaiAttWithMaiAttName.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.mai_att_name;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'mai_att_ins_par_status_id',
				dataIndex: 'mai_att_ins_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsMaiAttInsParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
