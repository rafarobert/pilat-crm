...
Ext.define('es.view.es-files.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esFiles').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esFiles').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esFiles').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esFiles').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'fil_par_type_id',
				dataIndex: 'fil_par_type_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsFilParTypeWithParCod.data.items;
                	let label;
			...
			{
				text: 'fil_par_status_id',
				dataIndex: 'fil_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsFilParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
