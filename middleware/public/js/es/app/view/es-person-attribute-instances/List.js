...
Ext.define('es.view.es-person-attribute-instances.List', {
	...
	//<es-section>
	store: 'esPersonAttributeInstances',
	//</es-section>
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esPersonAttributeInstances').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esPersonAttributeInstances').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esPersonAttributeInstances').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esPersonAttributeInstances').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'per_att_ins_group',
				dataIndex: 'per_att_ins_group',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'per_att_id',
				dataIndex: 'per_att_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsPersonAttributesPerAttWithPerAttName.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.per_att_name;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'per_att_ins_par_status_id',
				dataIndex: 'per_att_ins_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsPerAttInsParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
