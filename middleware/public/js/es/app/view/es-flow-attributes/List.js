...
Ext.define('es.view.es-flow-attributes.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esFlowAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esFlowAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esFlowAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esFlowAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'flo_att_description',
				dataIndex: 'flo_att_description',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsFlowsFloWithFloDescription.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.flo_description;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'flo_att_par_status_id',
				dataIndex: 'flo_att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsFloAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
