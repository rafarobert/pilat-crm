...
Ext.define('es.view.es-cron-attributes.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('esCronAttributes').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('esCronAttributes').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('esCronAttributes').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('esCronAttributes').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'cro_att_description',
				dataIndex: 'cro_att_description',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			
			{
				...
				renderer: (val, metaData, r) => {
                	let items = storeEsCronsCroWithCroDescription.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
                			label = item.raw.cro_description;
                		}
                	}
                	return label;
                },
			...
			{
				text: 'cro_att_par_status_id',
				dataIndex: 'cro_att_par_status_id',
				renderer: (val, metaData, r) => {
                	let items = storeEsParamsCroAttParStatusWithParCod.data.items;
                	let label;
                	for (let i = 0 ; i < items.length ; i++) {
                		let item = items[i];
                		if(item.raw._id == val) {
