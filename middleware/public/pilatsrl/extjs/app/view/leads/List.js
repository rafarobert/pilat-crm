 ...
 * Date: Thu Mar 11 2021 14:10:34 GMT-0400 (Bolivia Time)
 * Time: 14:10:34
 * Last User updated: Rafael Gutierrez Gaspar
 * Last date updated: Thu Mar 11 2021 14:10:34 GMT-0400 (Bolivia Time)
 * Last time updated: 14:10:34
 *
...
Ext.define('es.view.leads.List', {
	...
	initComponent: function () {
				...
				items: [
					{
						...
						handler: () => {
							...
							var date = new Date();
							//<es-section>
							var count = Ext.getStore('leads').data.length;
							var nextId = 0;
							for (var i = 0 ; i < count ; i++) {
								var id = Ext.getStore('leads').getAt(i).data.id;
								if(parseInt(id) > parseInt(nextId.toString())) {
									nextId = parseInt(id);
								}
							}
							Ext.getStore('leads').insert(count, {
								dueAt: date,
								createdAt: date,
								updatedAt: date,
							});
							Ext.getStore('leads').getAt(count).set('id',nextId+1);
							//</es-section>
						}
					}, '-' ,
					// {
		...
		this.columns = [
			...
			{
				text: 'opportunity_amount',
				dataIndex: 'opportunity_amount',
				width: 160,
				editor: {
					allowBlank: false
				}
			...
			{
				text: 'lead_source_description',
				dataIndex: 'lead_source_description',
				width: 160,
				editor: {
					allowBlank: false
				}
